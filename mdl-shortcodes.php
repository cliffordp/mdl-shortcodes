...
// string is h1-h6
// and if it is not, set a default value
function mdl_htag( $string, $default = 'h2' ) {
	$string = trim( strtolower( (string) $string ) );
...
if ( ! method_exists( 'MDL_Shortcodes\Shortcodes\Shortcode', 'mdl_sanitize_html_classes' ) && method_exists( 'MDL_Shortcodes\Shortcodes\Shortcode', 'sanitize_html_class' ) ) {
...
function mdl_icon_func( $atts ) {
	...
	if( method_exists( 'MDL_Shortcodes\Shortcodes\Shortcode', 'mdl_valid_icon_name_func' ) ) {
...
function mdl_cards_func( $atts ) {
	...
	$defaults = array(
		...
		'actions' => '', // bottom text, link(s), button(s), etc. -- defaults to "Read More" if 'actionslink' is 'true' and 'actions' is empty
		'actionslink' => 'true', // if using 'postid' and this is true, link the 'actions' text to the post as a button -- otherwise, just put full 'a href' HTML in 'actions' argument
	...
	$actionslink	=	strtolower( $atts['actionslink'] );
